...
# -*- coding: utf-8 -*-
"""
GRPO (Group Relative Policy Optimization) trainer - LoRA version
Online sampling + group-relative reward optimization + LoRA parameter-efficient fine-tuning
"""
...
@dataclass
class GRPOLoRAArguments(TrainingArguments):
    """GRPO LoRA training arguments"""
    # GRPO argument
    ...
    # multimodal learning-rate parameters
    mm_projector_lr: Optional[float] = field(default=None, metadata={"help": "Learning rate for multimodal projector"})
    vision_tower_lr: Optional[float] = field(default=None, metadata={"help": "Learning rate for vision tower"})
...
class OnlineGRPOLoRATrainer(Trainer):
    """
    Online GRPO trainer - LoRA version
    
    Workflow:
    1. Sample K responses for each prompt
    2. Score with the reward model
    3. Compute within-group relative advantages
    4. Optimize policy (update only LoRA parameters)
    """
    
    def __init__(
        self,
        model=None,
        grpo_args: GRPOLoRAArguments = None,
        ...
        **kwargs
        ...
        logger.info(f"GRPO LoRA configuration: samples={self.num_samples}, temp={self.temperature}, beta={self.beta}")
    ...
    def _save_sample_result(
        ...
        content = []
        content.append("=" * 80)
        content.append(f"GRPO LoRA sampling result - Step {step} | Prompt {prompt_idx} | Sample {sample_idx}")
    ...
    def sample_responses(
        ...
        # Sampling phase does not need gradients
        with torch.no_grad():
            for idx in range(batch_size):
                ...
                try:
                    ...
                    # 🚀 Batch-parallel sampling optimization
                        ...
                        try:
                            ...
                            if rank == 0:
                                self._save_sample_result(
                                    ...
                                    gen_time=batch_gen_time / self.num_samples,
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        """
        GRPO LoRA training step
        
        Step:
        1. Sample K responses for each prompt
        2. Score with the reward model
        3. Compute within-group relative advantages
        4. computepolicy-gradient loss and update (only update LoRA parameters)
        """
        # Extract data
        ...
        # Step 1: online sampling of multiple responses
        import torch.distributed as dist
        rank = dist.get_rank() if dist.is_initialized() else 0
        ...
        # Step 2: Compute GRPO loss (policy gradient)
        ...
        for batch_idx in range(len(prompts)):
            ...
            for response, reward in zip(responses, rewards):
                ...
                try:
                    ...
                    response_token_ids = input_ids_full[0, prompt_len:]
                    ...
                    for i in range(0, response_logits.shape[0], chunk_size):
                        ...
                        chunk_logits = response_logits[i:end_idx]
                        chunk_token_ids = response_token_ids[i:end_idx]
                    ...
                    # GRPO loss:-advantage * log_prob (normalized)
...
def train_grpo_online_lora():
    """Main training function - LoRA version"""
    
    # Parse arguments
    parser = HfArgumentParser((ModelArguments, DataArguments, GRPOLoRAArguments))
    model_args, data_args, training_args = parser.parse_args_into_dataclasses()
    
    # Load base model
    logger.info(f"Load base model: {model_args.model_name_or_path}")
    ...
    # Set model parameters (visual encoder, MLP, etc.)
    ...
    if model_args.use_lora:
        ...
        rank0_print(f"  - Rank: {model_args.lora_r}")
        rank0_print(f"  - Alpha: {model_args.lora_alpha}")
        rank0_print(f"  - Dropout: {model_args.lora_dropout}")
        rank0_print(f"  - Target Modules: {model_args.lora_target_modules}")
        ...
        rank0_print("="*80)
        rank0_print("LoRA model parameter statistics:")
        model.print_trainable_parameters()
        rank0_print("="*80)
    else:
        rank0_print("WARNING: LoRA is disabled. Using full fine-tuning.")
    ...
    # Create GRPO LoRA trainer
    trainer = OnlineGRPOLoRATrainer(
        model=model,
    ...
    # Start training
    logger.info("Start GRPO LoRA online training...")
    trainer.train()
    
    # Save LoRA adapter after training ends
    ...
    rank0_print("\n" + "="*80)
    rank0_print("tip:To use the trained model, you can:")
    rank0_print("1. directly load the LoRA adapter for inference")
    rank0_print("2. or merge LoRA weights into the base model")
    rank0_print(f"   python Q_merge_lora_weights.py \\")
    rank0_print(f"       --base_model {model_args.model_name_or_path} \\")
    rank0_print(f"       --lora_adapter {training_args.output_dir} \\")
    rank0_print(f"       --output_path {training_args.output_dir}_merged")
    rank0_print("="*80)
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("GRPO online training - LoRA version")
    print("="*80)
    print("\nLoRA advantages:")
    print("- Train only 1-5% of model parameters")
    print("- GPU memory requirement is greatly reduced")
    print("- training is faster")
    print("- multiple task-specific adapters can be saved")
    print("="*80)
    print()
    
    train_grpo_online_lora()
